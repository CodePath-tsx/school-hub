...
function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!user) { setErr("Invalid credentials."); setBusy(false); return; }
    nav({ to: "/app" });
  }
    ...
    <div className="min-h-screen grid md:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-6 md:p-12 bg-background">
        <form onSubmit={submit} className="w-full max-w-sm">
          ...
          <p className="text-center text-muted-foreground text-sm mt-1">Sign in to your account to continue</p>
          ...
          <div className="mt-8 space-y-4">
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input value={u} onChange={(e) => setU(e.target.value)} className="w-full pl-9 pr-3 h-10 rounded-md border bg-card outline-none focus:ring-2 focus:ring-ring/40" autoFocus />
              </div>
            </div>
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input type={show ? "text" : "password"} value={p} onChange={(e) => setP(e.target.value)} className="w-full pl-9 pr-9 h-10 rounded-md border bg-card outline-none focus:ring-2 focus:ring-ring/40" />
            ...
            <button disabled={busy} className="w-full h-11 rounded-md bg-primary text-primary-foreground font-medium hover:bg-primary/90 disabled:opacity-70 flex items-center justify-center gap-2">
