...
// the same shape is served by better-sqlite3 through the IPC bridge; the
// React code is unchanged.
...
export interface User {
  ...
  password: string; // demo only — Electron build uses bcrypt hashes
...
export interface Teacher {
  ...
  phone: string;
  subject: string;
  salaryMode: "session" | "percentage";
  salaryRate: number; // DA per session, or percentage 0-100
  status: "active" | "inactive";
}
...
export interface LicenseRecord {
  key: string;
  payload: import("./license").LicensePayload;
  machineId: string;
  activatedAt: string;
}
...
const STORAGE_KEY = "schoolbyte.db.v1";
...
function seed(): DB {
  ...
  const users: User[] = [
    { id: "u1", username: "admin", name: "Administrator", role: "admin", password: "admin", status: "active", createdAt: "2023-09-01", lastLogin: "2026-03-19T08:45:00" },
    { id: "u2", username: "sarah.sec", name: "Sarah Kaddouri", role: "secretary", password: "secret", status: "active", createdAt: "2023-09-01", lastLogin: "2026-03-18T14:20:00" },
    { id: "u3", username: "karim.sec", name: "Karim Belaid", role: "secretary", password: "secret", status: "inactive", createdAt: "2024-01-15", lastLogin: "2026-01-10T09:00:00" },
  ];
  return {
    users, teachers, rooms, groups, students,
    attendance: [],
    payments: [],
    expenses: [
      { id: "e1", label: "Rent", amount: 45000, category: "rent", at: "2026-03-01" },
      { id: "e2", label: "Utilities", amount: 12000, category: "utilities", at: "2026-03-01" },
    ],
    license: null,
    session: null,
    settings: {
      schoolName: "SchoolByte ERP",
      currency: "DA",
