...
/**
 * SchoolByte License Generator CLI (offline, Ed25519).
 *
 * Usage:
 *   node scripts/license-generator.mjs keygen
 *     → generates ~/.schoolbyte/keys/{private,public}.pem
 *       and prints the base64url public key to embed in src/lib/license.ts
 *
 *   node scripts/license-generator.mjs issue \
 *     --machine  1A3C72956371D459 \   ← Hardware Blueprint (HWID) from the Licence page
 *     --customer "Ahmed Ali" \
 *     --company  "ACME SARL" \
...
const KEYS_DIR = join(homedir(), ".schoolbyte", "keys");
...
function keygen() {
  ...
  console.log("\nPaste this into VENDOR_PUBLIC_KEY_B64URL in src/lib/license.ts:\n");
...
function issue(argv) {
  ...
  for (const r of required) {
    if (a[r] === undefined) { console.error(`Missing required argument: --${r}`); process.exit(1); }
  }
  ...
  const payload = {
    ...
    expiresAt: a.days
      ...
      : undefined,
    features: (a.features ?? "dashboard,students,teachers,groups,classrooms,attendance,statistics,users,settings").split(",").filter(Boolean),
